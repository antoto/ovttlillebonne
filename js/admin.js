/* ============================================================
   ...
   utilise des écouteurs "on" et non plus "once" : toute
   création/modification/suppression se répercute automatiquement
   ici ET sur le site public, sans recharger la page).

   Le bug d'affichage "un seul élément à la fois" (blog, calendrier,
   organisations) venait de js/firebase.js (voir le commentaire en
   tête de ce fichier) — corrigé, rien à changer ici pour ce point.
   ============================================================ */
...
document.querySelectorAll(".admin-modal-overlay").forEach((overlay) => {
  overlay.addEventListener("click", (e) => {
    ...
    // La modale de recadrage a besoin de nettoyer Cropper.js et de résoudre
    // sa promesse (voir finishCropper) : on route donc le clic extérieur
    // vers le bouton "Annuler" plutôt que de juste masquer la modale.
    if (overlay.id === "modal-cropper") { document.getElementById("cropper-cancel").click(); return; }
...
/* ============================================================
   OUTIL DE RECADRAGE D'IMAGE (Cropper.js)
   Utilisé pour toutes les photos importées dans l'administration
   (articles du blog, organisations…). Le fichier original n'est
   jamais envoyé tel quel : l'admin choisit la zone à garder dans
   une modale dédiée, puis seule l'image recadrée est envoyée vers
   Firebase Storage.
   ============================================================ */
let cropperInstance = null;
let cropperResolveFn = null;

function openCropper(file, aspectRatio) {
  return new Promise((resolve) => {
    if (!file || !file.type || !file.type.startsWith("image/")) {
      showToastAdmin("Fichier ignoré : ce n'est pas une image.");
      resolve(null);
      return;
    }
    ...
    reader.onload = (e) => {
      ...
      cropperResolveFn = resolve;
      // Cropper.js a besoin que l'image soit déjà dans le DOM et visible.
    ...
    reader.onerror = () => { showToastAdmin("Impossible de lire ce fichier."); resolve(null); };
...
function finishCropper(result) {
  ...
  const resolve = cropperResolveFn;
  cropperResolveFn = null;
  if (resolve) resolve(result);
}

document.getElementById("cropper-confirm").addEventListener("click", () => {
  if (!cropperInstance) { finishCropper(null); return; }
  cropperInstance.getCroppedCanvas({ maxWidth: 2000, maxHeight: 2000, imageSmoothingQuality: "high" }).toBlob((blob) => {
    finishCropper(blob);
  }, "image/jpeg", 0.9);
});
document.getElementById("cropper-cancel").addEventListener("click", () => finishCropper(null));

/* Branche un <input type="file"> sur le recadrage + upload Firebase,
   puis stocke l'URL obtenue dans un champ caché et affiche un aperçu.
   Utilisé pour les champs "image unique" (organisation). */
function setupSingleImageUploader({ inputId, previewId, hiddenId, storagePath, aspectRatio }) {
  const input = document.getElementById(inputId);
  input.addEventListener("change", async () => {
    const file = input.files[0];
    input.value = "";
    if (!file) return;
    const blob = await openCropper(file, aspectRatio);
    if (!blob) return;
    const preview = document.getElementById(previewId);
    preview.src = URL.createObjectURL(blob);
    preview.style.display = "block";
    showToastAdmin("Envoi de l'image…");
    FirebaseAPI.uploadImage(blob, storagePath, (url) => {
      document.getElementById(hiddenId).value = url;
      showToastAdmin("Image envoyée avec succès.");
    }, () => showToastAdmin("Erreur lors de l'envoi de l'image."));
  });
}
setupSingleImageUploader({
  inputId: "org-image-input", previewId: "org-image-preview",
  hiddenId: "org-image", storagePath: "organisations", aspectRatio: 16 / 10
});

/* ============================================================
   BLOG
   ============================================================ */
let articleImagesState = []; // URLs des photos de l'article en cours d'édition, dans l'ordre

function renderArticleImagesList() {
  const wrap = document.getElementById("article-images-list");
  if (!articleImagesState.length) {
    wrap.innerHTML = `<div class="fiche-empty">Aucune photo ajoutée pour le moment.</div>`;
    return;
  }
  wrap.innerHTML = articleImagesState.map((url, i) => `
    <div class="image-gallery-item">
      <img src="${url}" alt="Photo ${i + 1}">
      ${i === 0 ? '<span class="badge badge-blue">Couverture</span>' : ""}
      <div class="image-gallery-actions">
        ${i > 0 ? `<button type="button" class="btn-admin btn-admin-ghost" data-move-up="${i}" title="Monter">↑</button>` : ""}
        ${i < articleImagesState.length - 1 ? `<button type="button" class="btn-admin btn-admin-ghost" data-move-down="${i}" title="Descendre">↓</button>` : ""}
        <button type="button" class="btn-admin btn-admin-red" data-remove-image="${i}">Retirer</button>
      </div>
    </div>`).join("");

  wrap.querySelectorAll("[data-remove-image]").forEach((btn) => {
    btn.addEventListener("click", () => {
      articleImagesState.splice(parseInt(btn.dataset.removeImage, 10), 1);
      renderArticleImagesList();
    });
  });
  wrap.querySelectorAll("[data-move-up]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const i = parseInt(btn.dataset.moveUp, 10);
      [articleImagesState[i - 1], articleImagesState[i]] = [articleImagesState[i], articleImagesState[i - 1]];
      renderArticleImagesList();
    });
  });
...
document.getElementById("article-image-input").addEventListener("change", async () => {
  const input = document.getElementById("article-image-input");
  const file = input.files[0];
  input.value = "";
  if (!file) return;
  const blob = await openCropper(file, 16 / 10);
  if (!blob) return;
  showToastAdmin("Envoi de la photo…");
  FirebaseAPI.uploadImage(blob, "articles", (url) => {
    articleImagesState.push(url);
    renderArticleImagesList();
    showToastAdmin("Photo ajoutée à l'article.");
  }, () => showToastAdmin("Erreur lors de l'envoi de la photo."));
});

/* Insertion d'un lien cliquable dans le texte de l'article :
   sélectionnez du texte puis cliquez sur le bouton, ou cliquez sans
   rien sélectionner pour insérer un lien à personnaliser. Le lien
   est stocké au format Markdown [texte](url) et rendu cliquable
   automatiquement sur le site public (voir linkifyText dans main.js). */
document.getElementById("article-insert-link").addEventListener("click", () => {
  const ta = document.getElementById("article-texte");
  const start = ta.selectionStart, end = ta.selectionEnd;
  const selected = ta.value.slice(start, end);
  const url = prompt("Adresse du lien (https://...) :", "https://");
  if (!url) return;
  const label = selected || prompt("Texte affiché pour ce lien :", "cliquez ici") || url;
  ...
  FirebaseAPI.getArticles((list) => {
    ...
    tbody.querySelectorAll("[data-edit-article]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        articleImagesState = (a.images && a.images.length) ? [...a.images] : (a.image ? [a.image] : []);
        renderArticleImagesList();
...
document.getElementById("form-article").addEventListener("submit", (e) => {
  ...
  const data = {
    ...
    images: articleImagesState,
    image: articleImagesState[0] || ASSETS_BASE + "blog/article1.jpg",
...
/* ============================================================
   CALENDRIER
   ============================================================ */
const eventMultidayCheckbox = document.getElementById("event-multiday");
const eventDateFinWrap = document.getElementById("event-date-fin-wrap");
eventMultidayCheckbox.addEventListener("change", () => {
  const on = eventMultidayCheckbox.checked;
  eventDateFinWrap.style.display = on ? "block" : "none";
  document.getElementById("event-date-fin").required = on;
  if (!on) document.getElementById("event-date-fin").value = "";
});
  ...
  FirebaseAPI.getEvenements((list) => {
    ...
    tbody.innerHTML = sorted.map((ev) => `
      <tr>
        ...
        <td>${fmtDate(ev.date)}${ev.dateFin && ev.dateFin !== ev.date ? " → " + fmtDate(ev.dateFin) : ""}</td>
    ...
    tbody.querySelectorAll("[data-edit-event]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const isMultiday = !!ev.dateFin && ev.dateFin !== ev.date;
        eventMultidayCheckbox.checked = isMultiday;
        eventDateFinWrap.style.display = isMultiday ? "block" : "none";
        document.getElementById("event-date-fin").value = isMultiday ? ev.dateFin : "";
        document.getElementById("event-date-fin").required = isMultiday;
...
document.getElementById("add-event-btn").addEventListener("click", () => {
  ...
  eventMultidayCheckbox.checked = false;
  eventDateFinWrap.style.display = "none";
  document.getElementById("event-date-fin").required = false;
...
document.getElementById("form-event").addEventListener("submit", (e) => {
  ...
  const isMultiday = eventMultidayCheckbox.checked;
  const dateDebut = document.getElementById("event-date").value;
  const dateFin = isMultiday ? document.getElementById("event-date-fin").value : "";

  if (isMultiday && dateFin && dateFin < dateDebut) {
    showToastAdmin("La date de fin doit être postérieure à la date de début.");
    return;
  }
  ...
  const data = {
    ...
    date: dateDebut,
    // `null` supprime le champ côté Firebase (utile en mode "un seul jour")
    dateFin: (isMultiday && dateFin) ? dateFin : null,
...
/* ============================================================
   ORGANISATIONS
   ============================================================ */
let orgDocumentsState = []; // [{label, url}]

function renderOrgDocumentsList() {
  const wrap = document.getElementById("org-documents-list");
  if (!orgDocumentsState.length) {
    wrap.innerHTML = `<div class="fiche-empty">Aucun document ajouté pour le moment.</div>`;
    return;
  }
  wrap.innerHTML = orgDocumentsState.map((d, i) => `
    <div class="image-gallery-item image-gallery-item-doc">
      <span class="doc-chip">📄 ${escapeHtml(d.label)}</span>
      <div class="image-gallery-actions">
        <button type="button" class="btn-admin btn-admin-red" data-remove-doc="${i}">Retirer</button>
      </div>
    </div>`).join("");
  wrap.querySelectorAll("[data-remove-doc]").forEach((btn) => {
    btn.addEventListener("click", () => {
      orgDocumentsState.splice(parseInt(btn.dataset.removeDoc, 10), 1);
      renderOrgDocumentsList();
    });
  });
}

document.getElementById("org-document-input").addEventListener("change", () => {
  const input = document.getElementById("org-document-input");
  const file = input.files[0];
  input.value = "";
  if (!file) return;
  showToastAdmin("Envoi du document…");
  FirebaseAPI.uploadDocument(file, "organisations-documents", (url) => {
    orgDocumentsState.push({ label: file.name, url });
    renderOrgDocumentsList();
    showToastAdmin("Document ajouté.");
  }, () => showToastAdmin("Erreur lors de l'envoi du document."));
});

/* "Libellé | https://..." (un par ligne) -> [{label, url}, ...].
   Si aucun "|" n'est présent, l'URL sert aussi de libellé. */
function parseLinkLines(raw) {
  return (raw || "").split("\n").map((l) => l.trim()).filter(Boolean).map((line) => {
    const parts = line.split("|").map((s) => s.trim());
    if (parts.length >= 2 && parts[1]) return { label: parts[0], url: parts[1] };
  ...
  FirebaseAPI.getOrganisations((list) => {
    ...
    tbody.querySelectorAll("[data-edit-org]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const preview = document.getElementById("org-image-preview");
        ...
        document.getElementById("org-programme").value = o.programme || "";
        document.getElementById("org-infos").value = o.infos || "";
        document.getElementById("org-liens").value = linksToLines(o.liens);
        orgDocumentsState = Array.isArray(o.documents) ? [...o.documents] : [];
        renderOrgDocumentsList();
        openModal("modal-org");
      });
    });
...
document.getElementById("add-org-btn").addEventListener("click", () => {
  ...
  document.getElementById("org-image-preview").style.display = "none";
  orgDocumentsState = [];
  renderOrgDocumentsList();
...
document.getElementById("form-org").addEventListener("submit", (e) => {
  ...
  const data = {
    ...
    programme: document.getElementById("org-programme").value,
    infos: document.getElementById("org-infos").value,
    liens: parseLinkLines(document.getElementById("org-liens").value),
    documents: orgDocumentsState
  };
